在组合数学中，图论计数（Graph Enumeration）是研究满足特定性质的图的计数问题的分支。[生成函数](../多项式/intro.md)、[波利亚计数定理](polya.md) 与 [符号化方法](../多项式/symbolic-method.md#集合的-cycle-构造) 和 [OEIS](https://oeis.org/) 是解决这类问题时最重要的数学工具。图论计数可分为有标号和无标号两大类问题，大多数情况下[^1]有标号版本的问题都比其对应的无标号问题更加简单，因此我们将先考察有标号问题的计数。

[^1]: 也许无标号二叉树是一个反例，在结构简单的情况下，对应的置换群是恒等群（Identity Group），此时有标号版本可以直接通过乘以 $n!$ 得到。

## 有标号树

即 Cayley 公式，参见 [Prüfer 序列](../../graph/prufer.md) 一文，我们也可以使用 [Kirchhoff 矩阵树定理](../../graph/matrix-tree.md) 或 [生成函数](../多项式/intro.md#生成函数) 和 [拉格朗日定理](https://codeforces.com/blog/entry/104184) 得到这一结果。
...
代换进上式得到 $CG = H$，使用 [多项式求逆](../多项式/elementary-func.md#多项式求逆) 后再卷积解出 $C(x)$ 即可。

#### 方法三：多项式 exp

另一种做法是使用 [EGF 中多项式 exp 的组合意义](../多项式/egf.md#egf-中多项式-exp-的组合意义)，我们设有标号连通图和简单图序列的 EGF 分别为 $C(x)$ 和 $G(x)$，那么它们将有下列关系：
...
使用 [多项式 ln](../多项式/elementary-func.md#多项式对数函数--指数函数) 解出 $C(x)$ 即可。
...
[^2]: [粉兔的 blog](https://www.luogu.com.cn/blog/PinkRabbit/solution-sp4420) 告诉我们，这个序列也可以使用 [Chirp Z-Transform](../多项式/czt.md) 优化。
...
上述关于 EGF 的 exp 的用法，有时又被称作 Riddell's formula for labeled graphs，生成函数的 [欧拉变换](../多项式/symbolic-method.md#集合的-multiset-构造)，有时也被称为 Riddell's formula for unlabeled graphs，后者最早出现在欧拉对分拆数的研究中，除了解决图论计数问题之外，也在完全背包问题中出现。
