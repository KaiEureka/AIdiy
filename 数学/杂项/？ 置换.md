...
注意，本文讨论的不是排列数本文讨论的主题是全排列，而不是排列组合中的排列数。排列数的相关内容应当参考 [排列组合](./combinatorics/combination.md)。

## 定义

一个集合 $X$ 到自身的双射（即一一对应）$\sigma$ 称为 $X$ 的一个 **置换**（permutation）。如果集合 $X$ 上还具有 [全序](序理论.md#二元关系) 关系，则它的一个置换也常称作一个 **（全）排列**。这个全序关系称为集合上的自然顺序。
